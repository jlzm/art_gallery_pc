<template>
  <div class="class-assign">
    <div class="search" v-if="!isIndex">
      ...
      <div class="right">
        <el-button type="primary" icon="el-icon-refresh" size="small" :disabled="!canAutoAssign" @click="autoAssign">自动排课</el-button>
    ...
    <div class="dialog">
       <el-dialog :title="dialog.dialogTitle" :visible.sync="dialog.assignFormVisible" :close-on-click-modal="false" width="500px">
         <el-form
            :model="assignForm"
            class="form-inline"
            ref="assignForm"
            ...
            <el-form-item label="所需课时" prop="classNumber" >
              <el-input-number v-model="assignForm.classNumber" size="small" >
...
export default {
  ...
  methods: {
      ...
      arr.forEach((item, index) => {
        if (index === 0) {
          ...
          if (type === 'room') {
            ...
            if (item[type] === arr[index - 1][type] && this.spanObj.weeknum[index] === 0) {
              // 将上一个本来为0的置为1
    ...
    // 编辑单个教室
    editClass(row) {
      this.assignForm.treeValue = row.sid.split(',').filter((item) => {return item})
    ...
    getIndexData() {
      ...
      // 重新组装日期,保证不出现时分秒影响
      let todayTime = new Date(today.year + '-' + today.month + '-' + today.day).getTime()
    ...
    getClassRoomByWeek (data) {
      this.$axios.post('/getClassRoomByWeek',data)
        .then((res) => {
