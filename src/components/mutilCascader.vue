  ...
  <div class="mutil-cascader" :class="isLast &&  'last-child'">
    <div class="panel">
      <ul>
        ...
        >
          <div class="fa-label el-cascader-menu__item"
               :class="item.child && item.child.length && 'el-cascader-menu__item--extensible'">
            <!-- {{item.label}} -->
            <span v-if="!(item.child && item.child.length || isLast)">{{item.label}}</span>
            <el-checkbox :label="item.label" :checked="item.checked && !item.indeterminate"
                         @change.native="changeCheck(index)" :indeterminate="item.indeterminate" v-else></el-checkbox>
